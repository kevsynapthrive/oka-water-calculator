...
const appState = {
  ...
  targetYear: 8,
  includeReserveInRevenue: true, // Default to including reserves in revenue calculations
  ...
  currentBaseRate: 18.5, // Low base rate
  currentAddonFee: 7.25, // Infrastructure fee
  currentTiers: [
    { enabled: true, limit: 4000, rate: 5.2 }, // Very low rate for first tier
    { enabled: true, limit: 10000, rate: 5.8 }, // Minimal increase - no conservation incentive
    { enabled: false, limit: 15000, rate: 6.25 },
    { enabled: false, limit: 9999999, rate: 8.75 },
  ],

  // Future rate structure
  futureBaseRate: 18.5, // Higher base for fixed cost recovery
  futureAddonFee: 7.25, // Increased infrastructure fee
  futureTiers: [
    { enabled: true, limit: 3000, rate: 5.5 }, // Essential use at affordable rate
    { enabled: true, limit: 7000, rate: 7.2 }, // Average use
    { enabled: false, limit: 15000, rate: 11.5 }, // Higher usage - conservation pricing
    { enabled: false, limit: 9999999, rate: 15.75 }, // Luxury use - strong price signal
  ],
  // Calculation Current results
  currentResults: {
    tierBreakdown: [],
    affordabilityMHI: 0,
    revenuePieData: [],
  ...
  // Calculation What-if/Future results
  futureResults: {
    tierBreakdown: [],
    affordabilityMHI: 0,
    revenuePieData: [],
...
function setupEventListeners() {
  ...
  document
    .querySelectorAll('.card-header[data-bs-toggle="collapse"]')
    .forEach((header) => {
  ...
  document
    ...
    .addEventListener("click", addProjectEntry);
...
function setupInputChangeListeners() {
  ...
  document
    .getElementById("avgMonthlyUsage")
    .addEventListener("input", function () {
  ...
  document.getElementById("debtTerm").addEventListener("input", function () {
    ...
    console.log(
      "[DEBUG] appState.debtTerm after input parse:",
      appState.debtTerm
    );
    calculateAll();
  });
  ...
  document
    .getElementById("infrastructureCost")
    .addEventListener("input", function () {
      appState.infrastructureCost = parseFloat(this.value) || 0;
      calculateAll();
    });
  // Infrastructure reserve checkbox
  document.getElementById('includeReserveCheckbox').addEventListener('change', function() {
    appState.includeReserveInRevenue = this.checked;
    calculateAll();
  });
  // Sliders
  ...
  document
    .getElementById("projectionPeriod")
    .addEventListener("input", function () {
  ...
  const interestAdjustmentSlider = document.getElementById(
    "interestAdjustmentSlider"
  );
  ...
  // Current rate structure
  document
    .getElementById("currentBaseRate")
    .addEventListener("input", function () {
  ...
  document
    .getElementById("currentAddonFee")
    .addEventListener("input", function () {
  ...
  for (let i = 1; i <= 4; i++) {
    document
      .getElementById(`currentTier${i}Limit`)
      .addEventListener("input", function () {
    ...
    document
      .getElementById(`currentTier${i}Rate`)
      .addEventListener("input", function () {
  ...
  for (let i = 1; i <= 4; i++) {
    document
      .getElementById(`futureTier${i}Limit`)
      .addEventListener("input", function () {
    ...
    document
      .getElementById(`futureTier${i}Rate`)
      .addEventListener("input", function () {
...
function setupTierEnableDisableListeners() {
  ...
  for (let i = 1; i <= 4; i++) {
    document
      .getElementById(`currentTier${i}Enabled`)
  ...
  for (let i = 1; i <= 4; i++) {
    document
      .getElementById(`futureTier${i}Enabled`)
      .addEventListener("change", function () {
        const isEnabled = this.checked;
...
// /**
//  * Set input fields to match appState values -- DEPRICATED FUNCTION
//  */
...
function addTooltipsToInputs() {
  const tooltipInputs = [
    ...
    { id: "povertyIncome", tip: "Federal poverty level for your area" },
    {
      ...
      tip: "Percentage of households below poverty line",
    },
    { id: "customerCount", tip: "Total number of customer connections" },
    {
      id: "avgMonthlyUsage",
      tip: "Average monthly water usage per customer in gallons",
    },
    ...
    { id: "customerGrowthSlider", tip: "Expected annual customer growth rate" },
    {
      ...
      tip: "Annual adjustment to interest rate (if any)",
    },
  ...
  if (helpModal && helpButton && helpModalCloseBtn) {
    ...
    helpModalCloseBtn.addEventListener("keydown", function (event) {
      if (event.key === "Tab" && !event.shiftKey) {
        ...
        document
          .querySelector("#helpModal .modal-content")
          .querySelector(
